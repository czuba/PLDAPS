...
while p.trial.pldaps.iTrial < p.trial.pldaps.finish && p.trial.pldaps.quit~=2
    ...
    if ~p.trial.pldaps.quit
        ...
        if ~isempty(p.condMatrix)
            ...
            % On FIRST TRIAL, always use the currently queued up condMatrix
            % - only ONE condMatrix should control block alternation
            % - experimenter should always queue up that condMatrix for the first trial
            %   to ensure consistent initialization & block progression
            if nextTrial>1
                % otherwise, randomly select a condition matrix for this trial
                thisCondMat = randperm(numel(p.static.cm), 1);
                % assign to standard [active] position in pldaps object
                p.condMatrix = p.static.cm(thisCondMat);
            end
            % show this condMatrix stimulus name to info figure
            p.condMatrix.H.userPromptStr = p.condMatrix.stimName;
            ...
            % update list of module names associated with this condMatrix
            % - else will contain whatever was present during p.trial initialization
            p.updateModNames();
            ...
            % - across all [p.static.cm] condMatrix objects
            iPassMin = min([p.static.cm.iPass]);
            % ...clunky hack to manage inf
            maxPasses = [p.static.cm.nPasses];
            if ~all(isinf(maxPasses))
                maxPasses = max(maxPasses(~isinf(maxPasses)));                
                if p.condMatrix.iPass~=iPass && iPassMin>maxPasses
                    break
                end
            end
